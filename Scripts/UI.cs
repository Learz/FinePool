...
public class UI : Node {
    [Export]
    NodePath spinCursorPath;
    TextureRect spinCursor;
    [Export]
    NodePath powerBarPath;
    TextureProgress powerBar;

    // Called when the node enters the scene tree for the first time.
    public override void _Ready() {
        spinCursor = GetNode<TextureRect>(spinCursorPath);
        powerBar = GetNode<TextureProgress>(powerBarPath);
    }
